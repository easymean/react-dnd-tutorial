...
export const DropArea = ({
  ...
  const getClosestItem = (
    ...
    const closestItem = document
      .elementFromPoint(baseX, baseY)
      ?.closest('.drag-item');
    // 다중 draggable 방지
    if (closestItem && areaRef.current?.contains(closestItem)) {
      const closestRect = closestItem.getBoundingClientRect();
      let endX = closestRect.left;
      let endY = closestRect.top;
      const centerX = closestRect.left + closestRect.width / 2;
      const centerY = closestRect.top + closestRect.height / 2;
      if (baseline === 'behind') {
        endX += closestRect.width;
        endY += closestRect.height;
      }

      const isInRage = (val: number, min: number, max: number) => {
        if (min <= val && val <= max) {
          return true;
        } else {
          return false;
      ...
      const [minX, maxX] = [Math.min(endX, centerX), Math.max(endX, centerX)];
      const [minY, maxY] = [Math.min(endY, centerY), Math.max(endY, centerY)];

      if (
        (direction === 'vertical' && isInRage(baseY, minY, maxY)) ||
        (direction === 'horizontal' && isInRage(baseX, minX, maxX))
      ) {
        return closestItem;
      }
    }
  ...
  const mouseMoveHandler = (ev: MouseEvent) => {
    ...
    const baseline = (delta: number) => (delta > 0 ? 'behind' : 'ahead');

    const closestItem = getClosestItem(
      ghostRef.current,
      baseline(direction === 'vertical' ? deltaY : deltaX)
    ) as HTMLElement;
